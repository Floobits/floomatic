...
Listener.prototype.write = function(_path, md5, buf){
  var self = this,
    ...
    relpath = path.join(self.path, _path);

  if (existing && existing.md5 === md5){
    return;
  }

  fs.writeFile(relpath, buf, function(err){
    if (err){
      return console.error('could not write file: ' + relpath + ' because ' + toString(err));
    }
