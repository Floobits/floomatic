...
Listener.prototype.listener = function (event, rel_path) {
  ...
  if (event === 'rename') {
    ...
    //       self.conn.send_create_buf(rel_path, current);
  ...
  if (!buf) {
    ...
    return self.conn.send_create_buf({
      ...
      md5: md5,
      path: rel_path,
      encoding: utils.is_binary(current) ? "base64" : "utf8"
    });
  }
...
Listener.prototype.create_buf = function(_path, ig) {
  ...
  if (!stats.isDirectory()) {
    ...
    rel_path = path.rel_path(self.path, _path);
    if (self.bufs[rel_path] && self.bufs[rel_path].md5 == utils.md5(buf)) {
      return;
    }
    encoding = utils.is_binary(buf) ? 'base64' : 'utf8';

    return self.agent.write('create_buf', {
  ...
  if (!ig) {
    ig = ignores.build_ignores(_path);
  }
  ...
  _.each(paths, function(p) {
    ...
    if (ignored) {
      return log.log('Not creating buf', (ignored));
    }
    stats = fs.lstatSync(p_path);
    if (st.isDirectory()) {
      child_ig = ignores.Ignore(ig, p_path);
      setTimeout(self.create_buf.bind(self, p_path, child_ig), 0);
    } else {
      setTimeout(self.create_buf.bind(self, p_path, ig), 0);
    }
  });
  return;
};
...
Listener.prototype.inspect = function (cb) {
  var self = this,
    ignore = new Ignore(null, self.path);
...
Listener.prototype.patch = function (_path, patch_text, md5, id) {
  ...
  self.write(_path, res[0], md5, buf.encoding);
};
...
Listener.prototype.write = function (_path, buf, md5, encoding) {
  var self = this,
