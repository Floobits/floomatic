  ...
  if (event === 'rename') {
    ...
    if (current) {
      ...
      if (buf) {
        ...
        return self.conn.send_patch(buf, current);
      }
...
Listener.prototype.start_fs_watch = function () {
  var self = this;

  if (os.platform() === "darwin") {
    // TODO: use fsevents for darwin
    log.error("fs.watch can only watch 200ish files on darwin before exploding. Sorry :(");
    //return;
  }
...
Listener.prototype.create_buf = function (_path, ig) {
  ...
  try {
    ...
    log.error(util.format('Error readdiring %s: %s', _path, e2.toString()));
    return;
  }
  ...
  _.each(paths, function (p) {
    ...
    try {
      ...
      log.error(util.format('Error readdiring %s: %s', _path, e.toString()));
      return;
    }
    /*jslint stupid: true */
    stats = fs.lstatSync(p_path);
