...
const CLIENT = "floomatic";
const __VERSION__ = "0.11";
...
JS_DMP.Patch_DeleteThreshold = 0.375;
JS_DMP.Match_Threshold = 0.375;
JS_DMP.Match_Distance = 100;


const FlooConnection = function (_path, floorc, args) {
  var self = this;
  ...
  self.port = args.p;
  try {
    self.username = floorc.auth[self.host].username;
    self.api_key = floorc.auth[self.host].api_key;
    self.secret = floorc.auth[self.host].secret;
  } catch (e) {
    throw new Error(util.format("No entry found for %s in ~/.floorc.json", self.host));
  }
...
FlooConnection.prototype.send_patch = function (buf, after) {
  ...
  switch (buf.encoding) {
    ...
    case "base64":
      if (!DMP) {
        log.warn(util.format("Can't make patch for %s: No native-diff-match-patch module.", buf.path));
        return;
      }
      patch_text = DMP.patch_make(buf.buf, after);
      break;
    default:
      log.warn(util.format("Can't make patch for %s: Unknown encoding %s.", buf.path, buf.encoding));
      return;
  }
...
FlooConnection.prototype.on_room_info = function (d) {
  ...
  if (!_.includes(self.perms, "patch")) {
    if (!self.readonly) {
...
FlooConnection.prototype.on_ping = function (data) {
  var self = this;
  self.write("pong", data);
};
