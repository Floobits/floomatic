...
var FlooConnection = function (parsed_url, username, secret) {
  var self = this;
...
FlooConnection.prototype.connect = function (username, secret) {
  ...
  self.conn = self.parsed_url.klass.connect(parsed_url.port, parsed_url.host, function () {
    self.send_auth();
  });
...
FlooConnection.prototype.on_get_buf = function (buf) {
  var self = this,
    existing = self.listener.bufs[buf.path],
    relpath = path.join(self.listener.path, buf.path);

  if (existing && existing.md5 === buf.md5){
    return;
  }

  fs.writeFile(relpath, buf.buf, function(err){
    buf.buf = new Buffer(buf.buf);
    self.listener.bufs[buf.path] = buf;
    console.error(err);
