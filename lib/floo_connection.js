...
var FlooConnection = function (_path, floorc, args, on_room_info_cb) {
  var self = this;
  ...
  self.on_room_info_cb = on_room_info_cb;
...
FlooConnection.prototype.on_room_info = function (d) {
  ...
  _.each(d.bufs, function (buf, id) {
    ...
    if (buf.md5 !== md5) {
      ...
      if (self.create) {
        self.get_buf_cb[buf.id] = function (buf_id) {
  ...
  if (self.on_room_info_cb) {
    self.on_room_info_cb();
    self.on_room_info_cb = null;
  }
...
FlooConnection.prototype.on_create_buf = function (buf) {
  ...
  //TODO: don't stomp on our own files
  self.on_get_buf(buf);
};
