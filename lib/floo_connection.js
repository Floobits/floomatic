...
FlooConnection.prototype.start_syncing = function (listener) {
  var self = this,
    listener_bufs;
  ...
  _.each(self.room_info.bufs, function (buf, id) {
    ...
    if (existing.md5 !== buf.md5) {
      console.log("buf", existing.path, "md5 sum mismatch. re-fetching...");
      delete existing.buf;
      return self.send_get_buf(buf.id);
    }
  });
  ...
  _.each(self.listener.bufs, function (buf, _path) {
    if (!buf.id) {
      console.log("buf", _path, "doesn't exist. creating...");
