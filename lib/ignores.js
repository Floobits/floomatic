...
function Ignore(parent, path) {
  ...
  log.debug(util.format('Initializing ignores for %s', path));
...
Ignore.prototype.load = function (ignore_file) {
  ...
  /*jslint stupid: true */
  ignores = fs.readFileSync(path.join(self.path, ignore_file));
  /*jslint stupid: false */
...
Ignore.prototype.is_ignored = function (_path) {
  ...
  for (i=0; i<keys.length; i++) {
    ...
    for (j=0; j<patterns.length; j++) {
      ...
      if (pattern[0] === '/') {
        if (utils.unfuck_path(base_path) == self.path && minimatch(file_name, pattern.slice(1))) {
  ...
  if (self.parent) return self.parent.is_ignored(path);
